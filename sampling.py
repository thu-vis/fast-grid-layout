...
class UniformSampling():
    ...
    def fit(self, data):
        ...
        selected_indexes = np.random.choice(n, m, replace=False)
        indexer = faiss.IndexFlatL2(d)
        indexer.add(data[selected_indexes])
...
class HierarchySampling(object):
    ...
    def fit(self, data, category, sampling_rate, top_nodes_count):
        ...
        while len(selected_indexes)>top_nodes_count:
            ...
            faiss_sampler = UniformSampling(sampling_rate=sampling_number)
            new_selected_indexes, neighbors = faiss_sampler.fit(data[selected_indexes])
